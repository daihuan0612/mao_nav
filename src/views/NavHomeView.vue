...
const handleScroll = () => {
  const container = document.querySelector('.content-area')
  if (!container || !categories.value || categories.value.length === 0) return

  const scrollPosition = container.scrollTop + 140 // 增加偏移量以提前触发激活状态
...
.category-nav {
  padding: 16px 0;
  height: calc(100vh - 160px); /* 为底部留出空间 */
  overflow-y: auto;
}
